...
/** If nx_uniform_sampler_sample_s() has been called before this one, then the
 * *init* functions should be called again before calling this one
*/
double nx_uniform_sampler_sample_d(struct NXUniformSampler *sampler)
{
        return sfmt_genrand_res53(&sampler->state);
}
